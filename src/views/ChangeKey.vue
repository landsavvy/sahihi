    ...
    <div class="card shadow mb-4">
      ...
      <div class="card-body">
        ...
        <div class="form-group custom-file my-2">
          <input
            ref="fileNewPublicKey"
            type="file"
            @change="fileOpened"
            class="custom-file-input"
            id="customFile"
          />
          <label class="custom-file-label" for="customFile">Pick New Public Key</label>
          <!-- <small id="helpId" class="text-muted">New Public Key</small> -->
        </div>
        <div class="form-group custom-file my-2">
          <input
            ref="fileWitnessPrivateKey"
            type="file"
            @change="fileOpened"
            class="custom-file-input"
            id="customFile"
          />
          <label class="custom-file-label" for="customFile">Pick Witness Private Key</label>
          <!-- <small id="helpId" class="text-muted">Witness Private Key</small> -->
        </div>
...
export default {
  ...
  methods: {
    async exportSignature() {
      ...
      fs.writeFileSync(
        path.join(filepath, "ChangeKeySignature.json"),
        exportText
      );
      swal({
        title: "Saved",
    ...
    getWitnessKey() {
      return new Promise((resolve) => {
        var fileInput = this.$refs.fileWitnessPrivateKey;
    ...
    getPublicKey() {
      return new Promise((resolve) => {
        var fileInput = this.$refs.fileNewPublicKey;
        var file = fileInput.files[0];
    ...
    async sign() {
      ...
      this.witnessPrivateKey = await this.getWitnessKey();
      this.newPublicKey = await this.getPublicKey();
      ...
      signer.end();
      var signature = signer.sign(this.witnessPrivateKey, "base64");
      var finalFile = {
        signature,
        stringData,
      };
